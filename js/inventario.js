...
function rutProveedor(p){return p?`${p.rutCodigo||p.rut||''}${p.rutDV?'-'+p.rutDV:''}`:'—';}
function opcionesProveedor(sel=''){return '<option value="">Seleccione proveedor…</option>'+proveedoresOC().map(p=>`<option value="${esc(p.rutCodigo||p.rut)}" ${String(sel)===String(p.rutCodigo||p.rut)?'selected':''}>${esc(rutProveedor(p))} · ${esc(p.razonSocial||'')}</option>`).join('');}
...
function invNuevoMovimiento(){
  ...
  movDraft={tipo:'ENTRADA',folio:nuevoFolio('ENTRADA'),fecha:hoy(),motivo:'COMPRA',bodegaOrigenId:'',bodegaDestinoId:inv().bodegas.find(b=>b.activo!==false)?.id||'',tercero:'',documentoTipo:'SIN DOCUMENTO',documentoNumero:'',centroCosto:'',observaciones:'',lineas:[{id:uid(),productoId:'',cantidad:'',costoUnitario:'',lote:'',fechaVencimiento:''}]};invRenderMovModal();
}
function invEditarMovimiento(id){
  ...
  movDraft={editId:m.id,tipo:m.tipo,folio:m.folio,fecha:m.fecha,motivo:m.motivo,bodegaOrigenId:m.bodegaOrigenId||'',bodegaDestinoId:m.bodegaDestinoId||'',tercero:m.tercero||'',documentoTipo:m.documentoTipo||(m.documento?'OTRO':'SIN DOCUMENTO'),documentoNumero:m.documentoNumero||m.documento||'',centroCosto:m.centroCosto||'',observaciones:m.observaciones||'',lineas:(m.lineas||[]).map(l=>({...l}))};
  invRenderMovModal();
}
...
async function invGuardarMovimiento(){
  ...
  const now=new Date().toISOString(),m={tipo:movDraft.tipo,folio:movDraft.folio||nuevoFolio(movDraft.tipo),fecha:movDraft.fecha,motivo:movDraft.motivo,bodegaOrigenId:movDraft.bodegaOrigenId,bodegaDestinoId:movDraft.bodegaDestinoId,tercero:movDraft.tercero,documentoTipo:movDraft.documentoTipo,documentoNumero,documento,centroCosto:movDraft.centroCosto,observaciones:movDraft.observaciones,id:uid(),estado:'VIGENTE',creado:now,creadoPor:AUTH.user?.email||'',lineas:lineasFinal};
